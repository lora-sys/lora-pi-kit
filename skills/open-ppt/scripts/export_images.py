#!/usr/bin/env python3
"""Export a PPTD project as page images through the local editor mirror for visual QA.

Reuses the localhost editor host from export_pptx.py (fully offline), chooses
export -> images in the export dialog, captures the images ZIP, unzips it, and
stitches all pages into a single overview image that a multimodal model can review.
"""
...
# The export dialog's image format option is a plain <div class="radio-group-item">
# without an ARIA role, so agent-browser's interactive snapshot never exposes it.
# Local editor is same-origin; CDP Runtime.evaluate on the page works.
