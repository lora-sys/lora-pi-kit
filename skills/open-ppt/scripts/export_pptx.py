...
Optional --browser path: local editor via agent-browser
  (same UI as `npx open-ppt-skill serve`, fully offline).
...
EDITOR_MISSING_HINT = (
    "local editor not found. Re-run "
    "`npx open-ppt-skill install` (copies editor into the skill) "
    "or set OPEN_PPT_EDITOR to the editor directory."
)
...
def resolve_editor_root() -> Path:
    """Locate the offline editor mirror (package editor/ or skill-installed copy)."""
    env = os.environ.get("OPEN_PPT_EDITOR")
    candidates: List[Path] = []
    if env:
        candidates.append(Path(env).expanduser().resolve())
    candidates.append(SKILL_DIR / "editor")
    # monorepo / npm package: skills/open-ppt → package root
...
# Canonical WASM: editor/neo-ppt/assets/ (repo/npm). Skill install copies it
...
def export_pptx(
    ...
    """Prefer local WASM; fall back to local editor browser UI."""
    if prefer_local:
        try:
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Optional --browser uses the local editor (also offline)."
        )
    )
    ...
    parser.add_argument(
        "--browser",
        action="store_true",
        help="force local editor browser UI path instead of Node WASM",
    )
    parser.add_argument(
        "--keep-browser-raw",
        action="store_true",
