#!/usr/bin/env node
/**
 * Offline PPTD -> PPTX exporter using the official WASM PPTX writer (no webpage UI).
 *
 * What this does:
 *   1. Load a PPTD project from disk (manifest + pages + media)
 *   2. Resolve images to bytes
 *   3. Call the WASM writer: exportPPTDToPPTXBytes(pptd, options)
 *   4. Write .pptx
 *
 * Image ZIP export (pixel screenshots) still needs a DOM renderer; use --images
 * with Playwright against a local static render, or keep using the local editor for QA shots.
 *
 * Usage:
 *   node export-pptd.mjs <projectDirOr.pptd> -o out.pptx
 *   node export-pptd.mjs ./my-deck -o deck.pptx --no-sign
 *
 * Env:
 *   OPEN_PPT_EDITOR   path to editor directory (optional)
 */
...
// ---------- WASM glue (Node port of wasm-bindgen) ----------
...
Options:
  ...
  --no-sign             skip signature request (offline mode)
...
`);
  ...
  if (args.noSign) {
    ...
    console.warn('      --no-sign: using synthetic signature (offline mode)');
  }

  console.log('[3/5] load WASM', args.wasmPath);
  if (!fs.existsSync(args.wasmPath)) {
    throw new Error(
      `patched WASM not found: ${args.wasmPath}\n` +
        `Expected editor/neo-ppt/assets/${CANONICAL_WASM_NAME} (repo/npm package) ` +
  ...
  try {
    ...
    if (msg.includes('signature')) {
      throw new Error(
        `WASM export failed: ${msg}\n` +
          `Try running with --no-sign for offline mode.`,
      );
    }
    throw e;
  }
